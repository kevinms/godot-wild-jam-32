...
func _physics_process(delta):
	...
	#var dir = global_player_dir()
	#var look_target = $Pin.global_transform.origin + dir
	#$Pin.look_at(look_target, Vector3.UP)
...
#	var to_player = player_pos - $Pin.global_transform.origin
#	if to_player.length() < min_player_dist:
#		var velocity = to_player.normalized() * -speed
#		velocity = $Pin.move_and_slide(velocity, Vector3.UP)
...
onready var laser_scene = preload("res://DroneLaser.tscn")
