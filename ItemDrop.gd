...
var next_coin: float = 30.0
const max_coin: int = 1

func coin_drop(delta):
	next_coin -= delta
	if next_coin > 0:
		if Global.gold <= 0 and next_coin >= 15.0:
			next_coin = 15.0
		
		return
	next_coin = rand_range(45, 75.0)
	
	if get_tree().get_nodes_in_group("coin").size() >= max_coin:
		return
